import{C as be,c as ge}from"./validation-DnU8phWm.js";import{q as _e}from"./questions-D4Dn-9Ya.js";import{W as Y,S as oe}from"./witmeQuestions-Cl8VuAk9.js";const X={witme_featured:"Witme (oferta destacada)",...Object.fromEntries(be.map(e=>[e.id,e.name]))};function Q(e){if(e in X)return X[e];const t=e.match(/^witme_featured_(\d+)$/);return t?`Witme (oferta destacada ${t[1]})`:e}function de(e){const t={};for(const a of e)a.options&&(t[a.key]=Object.fromEntries(a.options.map(n=>[n.value,n.label])));return t}const fe=de(_e),ve=de(Y),he={si:"Sí",no:"No"};function $e(e,t,a){const n=e==="solicitud"?ve[t]:fe[t];return(n==null?void 0:n[a])??he[a]??a}const ye={ingreso_mensual:"Ingreso mensual",importe_total_de_la_deuda:"Deuda total (entre quienes tienen)",creditos_cantidad_a_solicitar:"Importe solicitado",age:"Edad",esta_en_asnef:"En ASNEF",antiguedad_laboral:"Antigüedad laboral",tienes_otros_creditos:"Tiene otras deudas",proposito_del_prestamo:"Propósito del préstamo",fuente_principal_de_ingreso:"Fuente de ingresos",tienes_vivienda_en_propiedad:"Vivienda en propiedad",en_cuantos_meses_deseas_devolverlo:"Plazo de devolución"},Se={monthlyIncome:"Ingreso mensual",totalDebtAmount:"Deuda total (entre quienes tienen)",requestedAmount:"Importe solicitado",numberOfdependents:"Personas a cargo",age:"Edad",incomeSource:"Fuente de ingresos",hasOwnedHouse:"Situación de vivienda",badCreditHistory:"En ASNEF",hasOtherLoans:"Tiene otras deudas",loanPurpose:"Propósito del préstamo",hasOwnVehicle:"Tiene vehículo propio",hasBankAccount:"Tiene cuenta bancaria",maritalStatus:"Estado civil",educationLevel:"Nivel de estudios",gender:"Género",countryOfBirth:"País de nacimiento",state:"Comunidad autónoma"},Ee=new Set(["ingreso_mensual","importe_total_de_la_deuda","creditos_cantidad_a_solicitar","monthlyIncome","totalDebtAmount","requestedAmount"]),ke="https://pgyaigdsedkdqvhtexrz.supabase.co",Le="sb_publishable_yL99vHU_H5kGZ3SMuPS0hA_GJ_TWTMr",b=ge(ke,Le),T="cs_admin_pw",p=document.getElementById("admin-root");function re(e){return e.toISOString().slice(0,10)}const ce=new Date;let h="all",M=re(ce),z=re(ce),f="all";const U=25;let v=0;const V=25;let L=0;const G=25;let I=0,j="dashboard";const Ie={base:"Quiz corto + Solicitud",ingreso_mensual:"Quiz corto + Solicitud",otros_creditos:"Quiz corto + Solicitud",asnef:"Quiz corto + Solicitud",ratio_deuda_ingreso:"Quiz corto + Solicitud",edad:"Quiz corto + Solicitud",fuente_ingreso:"Quiz corto",antiguedad_laboral:"Quiz corto",vivienda_propiedad:"Quiz corto",solicitud_fuente_ingreso:"Solicitud",solicitud_antiguedad:"Solicitud",solicitud_vivienda:"Solicitud",solicitud_dependientes:"Solicitud",aprobacion_base:"Probabilidad de aprobación (quiz + solicitud)",aprobacion_ratio_importe:"Probabilidad de aprobación (quiz + solicitud)"},Z={all:"Todos",quiz:"Quiz corto",solicitud:"Solicitud completa"};function B(e){const t=new Date;if(e==="today")return{since:new Date(t.getFullYear(),t.getMonth(),t.getDate(),0,0,0,0).toISOString(),until:t.toISOString()};if(e==="7d")return{since:new Date(t.getTime()-6048e5).toISOString(),until:t.toISOString()};if(e==="custom"){const a=new Date(`${M}T00:00:00`),n=new Date(`${z}T23:59:59.999`);return a.getTime()>n.getTime()?{since:n.toISOString(),until:a.toISOString()}:{since:a.toISOString(),until:n.toISOString()}}return{since:"2000-01-01T00:00:00.000Z",until:t.toISOString()}}const ee=new Intl.DateTimeFormat("es-ES",{day:"2-digit",month:"short",year:"numeric"});function we(e,t){return e==="all"?"Todo el histórico":`${ee.format(new Date(t.since))} – ${ee.format(new Date(t.until))}`}const Te=[{key:"fecha_de_nacimiento",label:"Fecha de nacimiento"},{key:"codigo_postal",label:"Código postal"},{key:"fuente_principal_de_ingreso",label:"Fuente de ingresos"},{key:"antiguedad_laboral",label:"Antigüedad laboral",conditional:!0},{key:"tienes_vivienda_en_propiedad",label:"Vivienda en propiedad"},{key:"ingreso_mensual",label:"Ingreso mensual"},{key:"esta_en_asnef",label:"Asnef"},{key:"tienes_otros_creditos",label:"Otros créditos"},{key:"importe_total_de_la_deuda",label:"Importe de la deuda",conditional:!0},{key:"proposito_del_prestamo",label:"Propósito del préstamo"},{key:"creditos_cantidad_a_solicitar",label:"Importe a solicitar"},{key:"en_cuantos_meses_deseas_devolverlo",label:"Plazo de devolución"}],Ce=Y.filter(e=>oe.includes(e.phase)),Ae=Y.filter(e=>!oe.includes(e.phase)),te=e=>({key:e.key,label:e.label,conditional:!!e.condition}),qe=[...Ce.map(te),{key:"gate_contact",label:"Deja sus datos de contacto (nombre, email, teléfono)"},...Ae.map(te),{key:"application_completed",label:"✅ Termina la solicitud completa"}],W=new Intl.DateTimeFormat("es-ES",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"});function o(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML.replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function O(e){return e==="all"?null:e}async function le(e,t,a){const{data:n,error:i}=await b.rpc("admin_get_stats",{p_password:e,p_since:t.since,p_until:t.until,p_source:O(a)}).single();if(i||!n)throw i??new Error("No data");return n}async function Oe(e,t,a){const{data:n,error:i}=await b.rpc("admin_get_funnel_overview",{p_password:e,p_since:t.since,p_until:t.until,p_source:O(a)}).single();if(i||!n)throw i??new Error("No data");return n}async function Pe(e,t,a){const{data:n,error:i}=await b.rpc("admin_get_funnel_steps",{p_password:e,p_since:t.since,p_until:t.until,p_source:O(a)});if(i)throw i;return n??[]}async function De(e,t,a,n){const{data:i,error:d}=await b.rpc("admin_list_leads",{p_password:e,p_limit:U,p_offset:n*U,p_since:t.since,p_until:t.until,p_source:O(a)});if(d)throw d;return i??[]}async function je(e,t){const{data:a,error:n}=await b.rpc("admin_get_witme_applications",{p_password:e,p_limit:V,p_offset:t*V});if(n)throw n;return a??[]}async function Re(e,t){const{data:a,error:n}=await b.rpc("admin_get_witme_car_applications",{p_password:e,p_limit:G,p_offset:t*G});if(n)throw n;return a??[]}async function Me(e){const{data:t,error:a}=await b.rpc("admin_get_witme_response_stats",{p_password:e}).single();if(a||!t)throw a??new Error("No data");return t}function A(e){return e==null?"—":`${(e/1e3).toFixed(1)} s`}function ae(e){return e==null?"—":`${e}%`}async function ze(e,t,a){const{data:n,error:i}=await b.rpc("admin_get_offer_clicks",{p_password:e,p_since:t.since,p_until:t.until,p_source:O(a)});if(i)throw i;return n??[]}let ue=[];async function xe(e){const{data:t,error:a}=await b.rpc("admin_get_scoring_rules",{p_password:e});if(a)throw a;return t??[]}async function Be(e,t,a,n,i){const{error:d}=await b.rpc("admin_update_scoring_rule",{p_password:e,p_key:t,p_config:a,p_weight:n,p_active:i});if(d)throw d}async function Fe(e,t){const{error:a}=await b.rpc("admin_reset_scoring_rule",{p_password:e,p_key:t});if(a)throw a}async function He(e){const{error:t}=await b.rpc("admin_reset_all_scoring_rules",{p_password:e});if(t)throw t}async function Ne(e,t,a){const{data:n,error:i}=await b.rpc("admin_get_field_stats",{p_password:e,p_since:t.since,p_until:t.until,p_source:a});if(i)throw i;return n}function C(e){p.innerHTML=`
  ...
  `,document.getElementById("login-form").addEventListener("submit",async t=>{t.preventDefault();const a=document.getElementById("pw-input").value;try{await le(a,B("all"),"all"),sessionStorage.setItem(T,a),w(a)}catch{C("Contraseña incorrecta.")}})}function l(e,t){return`<div class="admin-stat"><span class="admin-stat-value">${t}</span><span class="admin-stat-label">${e}</span></div>`}function P(e,t,a,n){const i=a>0?Math.round(t/a*100):0;return`
  ...
  `}function ne(e){return e>=60?"band-excelente":e>=35?"band-bueno":e>=15?"band-regular":"band-bajo"}function We(e,t,a){var c;const n=new Map(t.map(u=>[u.question_key,Number(u.reached)])),i=e.engaged_visits;let d="",r=(c=a[0])==null?void 0:c.key;return a.forEach((u,y)=>{const g=n.get(u.key)??0,_=i>0?Math.round(g/i*100):0;let m="";if(y>0&&!u.conditional){const E=n.get(r)??0;if(E>0){const k=Math.round((1-g/E)*100),s=k>=25?"high":k>=10?"mid":"low";m=k>0?`<span class="funnel-drop funnel-drop-${s}">-${k}% respecto al paso anterior</span>`:'<span class="funnel-drop funnel-drop-low">sin caída</span>'}}d+=`
    ...
    `,u.conditional||(r=u.key)}),d}function w(e){j==="scoring"?x(e):j==="fieldstats"?pe(e):j==="leads"?S(e):me(e)}function F(e){return`
  ...
  `}function H(e){document.querySelectorAll(".admin-tab-btn").forEach(t=>{t.addEventListener("click",()=>{j=t.dataset.tab,w(e)})}),document.getElementById("refresh-btn").addEventListener("click",()=>w(e)),document.getElementById("logout-btn").addEventListener("click",()=>{sessionStorage.removeItem(T),C()})}function J(e){return`
    <section class="admin-card admin-source-bar">
      <span class="admin-source-label">Embudo:</span>
      <div class="admin-period-presets">
        ${Object.keys(Z).map(t=>`<button class="admin-period-btn ${f===t?"active":""}" data-source="${t}">${Z[t]}</button>`).join("")}
      </div>
    </section>
    ...
    <section class="admin-card admin-period-bar">
      ...
      <div class="admin-period-custom ${h==="custom"?"active":""}">
        <input type="date" id="period-from" value="${M}" />
        <span>–</span>
  ...
  `}function K(e){var t;document.querySelectorAll(".admin-period-btn[data-source]").forEach(a=>{a.addEventListener("click",()=>{f=a.dataset.source,v=0,w(e)})}),document.querySelectorAll(".admin-period-btn[data-preset]").forEach(a=>{a.addEventListener("click",()=>{h=a.dataset.preset,v=0,w(e)})}),(t=document.getElementById("period-apply-btn"))==null||t.addEventListener("click",()=>{M=document.getElementById("period-from").value||M,z=document.getElementById("period-to").value||z,h="custom",v=0,w(e)})}async function me(e){p.innerHTML='<div class="admin-shell"><p class="admin-loading">Cargando…</p></div>';const t=B(h);try{const[a,n,i,d]=await Promise.all([le(e,t,f),Oe(e,t,f),Pe(e,t,f),ze(e,t,f)]),r=a.band_excelente+a.band_bueno+a.band_regular+a.band_bajo,c=f==="solicitud"?qe:Te;p.innerHTML=`
      <div class="admin-shell">
        ...
        <section class="admin-card">
          ...
          ${P("Excelente",a.band_excelente,r,"band-excelente")}
          ${P("Bueno",a.band_bueno,r,"band-bueno")}
          ${P("Regular",a.band_regular,r,"band-regular")}
          ${P("Bajo",a.band_bajo,r,"band-bajo")}
        </section>
    ...
    `,H(e),K(e)}catch(a){const n=a instanceof Error?a.message:String(a);n.toLowerCase().includes("unauthorized")?(sessionStorage.removeItem(T),C("Tu sesión ha caducado o la contraseña ya no es válida.")):(p.innerHTML=`
        <div class="admin-shell">
      ...
      `,document.getElementById("retry-btn").addEventListener("click",()=>me(e)))}}async function S(e){var a,n,i;p.innerHTML='<div class="admin-shell"><p class="admin-loading">Cargando…</p></div>';const t=B(h);try{const[d,r,c,u]=await Promise.all([De(e,t,f,v),je(e,L),Me(e),Re(e,I)]),y=((a=d[0])==null?void 0:a.total_count)??0,g=Math.max(1,Math.ceil(y/U)),_=((n=r[0])==null?void 0:n.total_count)??0,m=Math.max(1,Math.ceil(_/V)),E=((i=u[0])==null?void 0:i.total_count)??0,k=Math.max(1,Math.ceil(E/G));p.innerHTML=`
      <div class="admin-shell">
        ${F("leads")}
              ...
              <tbody>
                ${d.map(s=>`
                  <tr>
                    ...
                    <td>${s.approval_probability!=null?`<span class="admin-badge ${ne(s.approval_probability)}">${s.approval_probability}%</span>`:"—"}</td>
                    <td>${o(s.status)}</td>
                    <td>${o(Z[s.source]??s.source)}</td>
                    ...
                    <td>${s.offer_clicks&&s.offer_clicks.length>0?s.offer_clicks.map($=>o(Q($))).join(", "):"—"}</td>
                  </tr>
                `).join("")}
                ${d.length===0?'<tr><td colspan="12" class="admin-empty">Todavía no hay leads.</td></tr>':""}
              </tbody>
            </table>
          </div>
        ...
        <section class="admin-card">
          ...
          <section class="admin-stats-grid admin-stats-grid-compact">
            ${l("% Aceptados (con oferta)",ae(c.pct_accepted))}
            ${l("% Rechazados por Witme",ae(c.pct_failed))}
          ...
          <div class="admin-table-scroll">
            <table class="admin-table">
              ...
              <tbody>
                ${r.map(s=>{const $=[];s.witme_message!=null&&$.push(`Mensaje: ${JSON.stringify(s.witme_message)}`),s.witme_redirect_url&&$.push(`Redirect URL: ${s.witme_redirect_url}`);const q=$.join(`
`);return`
                  <tr>
                    <td>${W.format(new Date(s.created_at))}</td>
                    ...
                    <td>${s.score??"—"}</td>
                    <td>${s.approval_probability!=null?`<span class="admin-badge ${ne(s.approval_probability)}">${s.approval_probability}%</span>`:"—"}</td>
                    <td>${s.offer_clicks&&s.offer_clicks.length>0?`✅ ${s.offer_clicks.map(N=>o(Q(N))).join(", ")}`:"—"}</td>
                  </tr>
                `}).join("")}
                ${r.length===0?'<tr><td colspan="10" class="admin-empty">Todavía no hay solicitudes enviadas a Witme.</td></tr>':""}
              </tbody>
            </table>
          </div>
          <div class="admin-pagination">
        ...
        <section class="admin-card">
          <p class="admin-card-title">Solicitudes enviadas a Witme · aval coche / reunificación (${E})</p>
          <p class="admin-card-sub">
...
`),N=s.product==="car_collateral+debt_consolidation"?"Aval coche + Reunificación deudas":s.product==="car_collateral"?"Aval coche":s.product==="debt_consolidation"?"Reunificación deudas":"—";return`
                ...
                `}).join("")}
                ${u.length===0?'<tr><td colspan="8" class="admin-empty">Todavía no hay solicitudes de estos productos.</td></tr>':""}
              </tbody>
            </table>
          </div>
          <div class="admin-pagination">
    ...
    `,H(e),K(e),document.getElementById("leads-prev-btn").addEventListener("click",()=>{v>0&&(v--,S(e))}),document.getElementById("leads-next-btn").addEventListener("click",()=>{v++,S(e)}),document.getElementById("witme-prev-btn").addEventListener("click",()=>{L>0&&(L--,S(e))}),document.getElementById("witme-next-btn").addEventListener("click",()=>{L++,S(e)}),document.getElementById("witme-car-prev-btn").addEventListener("click",()=>{I>0&&(I--,S(e))}),document.getElementById("witme-car-next-btn").addEventListener("click",()=>{I++,S(e)})}catch(d){const r=d instanceof Error?d.message:String(d);r.toLowerCase().includes("unauthorized")?(sessionStorage.removeItem(T),C("Tu sesión ha caducado o la contraseña ya no es válida.")):(p.innerHTML=`
        <div class="admin-shell">
          <p class="admin-error">Ha ocurrido un error inesperado cargando el panel: ${o(r)}</p>
          <button class="admin-btn-ghost" id="retry-btn">Reintentar</button>
        </div>
      `,document.getElementById("retry-btn").addEventListener("click",()=>S(e)))}}function Qe(e,t,a){return a?`${e} +`:`${e} – ${t}`}function R(e,t,a,n,i,d,r){return`
  ...
  `}function Ue(e){const t=e.config;if(typeof t.value=="number"&&Object.keys(t).length===1)return R(e.key,"value","Puntos base",t.value,300,850,5);if(Array.isArray(t.buckets)){const a=t.buckets;return a.map((n,i)=>R(e.key,`bucket:${i}`,Qe(n[0],n[1],i===a.length-1),n[2],-200,200,5)).join("")}return Object.entries(t).map(([a,n])=>R(e.key,`opt:${a}`,a,Number(n),-200,200,5)).join("")}function Ve(e){return`
    <div class="scoring-rule-card" data-rule-card="${e.key}">
      ...
      ${R(e.key,"weight","Peso (multiplica todos los puntos de esta regla)",Number(e.weight),0,3,.1)}
  ...
  `}function Ge(e){var t;document.querySelectorAll(".scoring-slider").forEach(a=>{a.addEventListener("input",()=>{var i;const n=(i=a.closest(".scoring-slider-row"))==null?void 0:i.querySelector(".scoring-slider-value");n&&(n.textContent=a.value)})}),document.querySelectorAll("[data-save-rule]").forEach(a=>{a.addEventListener("click",async()=>{const n=a.dataset.saveRule,i=ue.find(m=>m.key===n),d=document.querySelector(`[data-rule-card="${n}"]`);if(!i||!d)return;const r=d.querySelector(".scoring-rule-status"),c=new Map;d.querySelectorAll("input[data-field]").forEach(m=>{c.set(m.dataset.field,m.type==="checkbox"?String(m.checked):m.value)});const u=Number(c.get("weight")),y=c.get("active")==="true",g=i.config;let _;typeof g.value=="number"&&Object.keys(g).length===1?_={value:Number(c.get("value"))}:Array.isArray(g.buckets)?_={buckets:g.buckets.map((m,E)=>[m[0],m[1],Number(c.get(`bucket:${E}`))])}:(_={},Object.keys(g).forEach(m=>{_[m]=Number(c.get(`opt:${m}`))})),a.disabled=!0,r.textContent="Guardando…",r.className="scoring-rule-status";try{await Be(e,n,_,u,y),i.config=_,i.weight=u,i.active=y,r.textContent="✓ Guardado",r.className="scoring-rule-status ok",setTimeout(()=>{r.textContent=""},2500)}catch{r.textContent="Error al guardar",r.className="scoring-rule-status error"}finally{a.disabled=!1}})}),document.querySelectorAll("[data-reset-rule]").forEach(a=>{a.addEventListener("click",async()=>{const n=a.dataset.resetRule,i=document.querySelector(`[data-rule-card="${n}"]`);if(!i||!confirm("¿Restaurar esta regla a sus valores por defecto? Se aplicará de inmediato."))return;const d=i.querySelector(".scoring-rule-status");a.disabled=!0,d.textContent="Restaurando…",d.className="scoring-rule-status";try{await Fe(e,n),await x(e)}catch{d.textContent="Error al restaurar",d.className="scoring-rule-status error",a.disabled=!1}})}),(t=document.getElementById("reset-all-rules-btn"))==null||t.addEventListener("click",async()=>{if(confirm("¿Restaurar TODAS las reglas de scoring a sus valores por defecto? Esto sobrescribe cualquier ajuste manual y se aplica de inmediato a las puntuaciones reales."))try{await He(e),await x(e)}catch{alert("No se ha podido restaurar. Inténtalo de nuevo.")}})}async function x(e){p.innerHTML='<div class="admin-shell"><p class="admin-loading">Cargando…</p></div>';try{const t=await xe(e);ue=t,p.innerHTML=`
      <div class="admin-shell">
        ${F("scoring")}
    ...
    `,H(e),Ge(e)}catch(t){const a=t instanceof Error?t.message:String(t);a.toLowerCase().includes("unauthorized")?(sessionStorage.removeItem(T),C("Tu sesión ha caducado o la contraseña ya no es válida.")):(p.innerHTML=`
      ...
      `,document.getElementById("retry-btn").addEventListener("click",()=>x(e)))}}const se=new Intl.NumberFormat("es-ES",{maximumFractionDigits:1});function D(e,t){return e==null?"—":t?`${se.format(e)} €`:se.format(e)}function Ze(e,t,a){const n=Ee.has(t);return`
  ...
  `}function Ye(e,t,a,n){const i=t.reduce((d,r)=>d+r.count,0);return`
    <div class="fieldstat-card">
      <p class="fieldstat-label">${o(e)}</p>
      ${t.map(d=>{const r=i>0?Math.round(d.count/i*100):0;return`
            <div class="admin-band-row">
              <span class="admin-band-label">${o($e(a,n,d.value))}</span>
  ...
  `}function Je(e,t){const a=e==="solicitud"?Se:ye;return`
    <section class="admin-card">
      ...
      <div class="fieldstats-grid">
        ${Object.entries(t.numeric).map(([i,d])=>Ze(a[i]??i,i,d)).join("")}
        ${Object.entries(t.categorical).map(([i,d])=>Ye(a[i]??i,d,e,i)).join("")}
      </div>
    </section>
  `}async function pe(e){p.innerHTML='<div class="admin-shell"><p class="admin-loading">Cargando…</p></div>';const t=B(h),a=f==="solicitud"?["solicitud"]:f==="quiz"?["quiz"]:["quiz","solicitud"];try{const n=await Promise.all(a.map(i=>Ne(e,t,i)));p.innerHTML=`
      <div class="admin-shell">
        ${F("fieldstats")}
        ...
        ${a.map((i,d)=>Je(i,n[d])).join("")}
      </div>
    `,H(e),K(e)}catch(n){const i=n instanceof Error?n.message:String(n);i.toLowerCase().includes("unauthorized")?(sessionStorage.removeItem(T),C("Tu sesión ha caducado o la contraseña ya no es válida.")):(p.innerHTML=`
        <div class="admin-shell">
          <p class="admin-error">Ha ocurrido un error inesperado cargando las estadísticas: ${o(i)}</p>
          <button class="admin-btn-ghost" id="retry-btn">Reintentar</button>
        </div>
      `,document.getElementById("retry-btn").addEventListener("click",()=>pe(e)))}}const ie=sessionStorage.getItem(T);ie?w(ie):C();
