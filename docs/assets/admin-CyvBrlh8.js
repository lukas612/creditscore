import{C as re,c as de}from"./validation-DnU8phWm.js";import{q as ce}from"./questions-D4Dn-9Ya.js";import{W as K}from"./witmeQuestions-D841kSbQ.js";const B={witme_featured:"Witme (oferta destacada)",...Object.fromEntries(re.map(e=>[e.id,e.name]))};function X(e){const t={};for(const a of e)a.options&&(t[a.key]=Object.fromEntries(a.options.map(s=>[s.value,s.label])));return t}const le=X(ce),ue=X(K),me={si:"Sí",no:"No"};function pe(e,t,a){const s=e==="solicitud"?ue[t]:le[t];return(s==null?void 0:s[a])??me[a]??a}const be={ingreso_mensual:"Ingreso mensual",importe_total_de_la_deuda:"Deuda total (entre quienes tienen)",creditos_cantidad_a_solicitar:"Importe solicitado",age:"Edad",esta_en_asnef:"En ASNEF",antiguedad_laboral:"Antigüedad laboral",tienes_otros_creditos:"Tiene otras deudas",proposito_del_prestamo:"Propósito del préstamo",fuente_principal_de_ingreso:"Fuente de ingresos",tienes_vivienda_en_propiedad:"Vivienda en propiedad",en_cuantos_meses_deseas_devolverlo:"Plazo de devolución"},ge={monthlyIncome:"Ingreso mensual",totalDebtAmount:"Deuda total (entre quienes tienen)",requestedAmount:"Importe solicitado",numberOfdependents:"Personas a cargo",age:"Edad",incomeSource:"Fuente de ingresos",hasOwnedHouse:"Situación de vivienda",badCreditHistory:"En ASNEF",hasOtherLoans:"Tiene otras deudas",loanPurpose:"Propósito del préstamo",hasOwnVehicle:"Tiene vehículo propio",hasBankAccount:"Tiene cuenta bancaria",maritalStatus:"Estado civil",educationLevel:"Nivel de estudios",gender:"Género",countryOfBirth:"País de nacimiento",state:"Comunidad autónoma"},_e=new Set(["ingreso_mensual","importe_total_de_la_deuda","creditos_cantidad_a_solicitar","monthlyIncome","totalDebtAmount","requestedAmount"]),fe="https://pgyaigdsedkdqvhtexrz.supabase.co",ve="sb_publishable_yL99vHU_H5kGZ3SMuPS0hA_GJ_TWTMr",_=de(fe,ve),w="cs_admin_pw",v=document.getElementById("admin-root");function ee(e){return e.toISOString().slice(0,10)}const te=new Date;let h="all",j=ee(te),z=ee(te),g="all";const F=25;let f=0;const R=25;let E=0,H="dashboard";const he={base:"Quiz corto + Solicitud",ingreso_mensual:"Quiz corto + Solicitud",otros_creditos:"Quiz corto + Solicitud",asnef:"Quiz corto + Solicitud",ratio_deuda_ingreso:"Quiz corto + Solicitud",edad:"Quiz corto + Solicitud",fuente_ingreso:"Quiz corto",antiguedad_laboral:"Quiz corto",vivienda_propiedad:"Quiz corto",solicitud_fuente_ingreso:"Solicitud",solicitud_antiguedad:"Solicitud",solicitud_vivienda:"Solicitud",solicitud_dependientes:"Solicitud",aprobacion_base:"Probabilidad de aprobación (quiz + solicitud)",aprobacion_ratio_importe:"Probabilidad de aprobación (quiz + solicitud)"},M={all:"Todos",quiz:"Quiz corto",solicitud:"Solicitud completa"};function N(e){const t=new Date;if(e==="today")return{since:new Date(t.getFullYear(),t.getMonth(),t.getDate(),0,0,0,0).toISOString(),until:t.toISOString()};if(e==="7d")return{since:new Date(t.getTime()-6048e5).toISOString(),until:t.toISOString()};if(e==="custom"){const a=new Date(`${j}T00:00:00`),s=new Date(`${z}T23:59:59.999`);return a.getTime()>s.getTime()?{since:s.toISOString(),until:a.toISOString()}:{since:a.toISOString(),until:s.toISOString()}}return{since:"2000-01-01T00:00:00.000Z",until:t.toISOString()}}const V=new Intl.DateTimeFormat("es-ES",{day:"2-digit",month:"short",year:"numeric"});function $e(e,t){return e==="all"?"Todo el histórico":`${V.format(new Date(t.since))} – ${V.format(new Date(t.until))}`}const ye=[{key:"fecha_de_nacimiento",label:"Fecha de nacimiento"},{key:"codigo_postal",label:"Código postal"},{key:"fuente_principal_de_ingreso",label:"Fuente de ingresos"},{key:"antiguedad_laboral",label:"Antigüedad laboral",conditional:!0},{key:"tienes_vivienda_en_propiedad",label:"Vivienda en propiedad"},{key:"ingreso_mensual",label:"Ingreso mensual"},{key:"esta_en_asnef",label:"Asnef"},{key:"tienes_otros_creditos",label:"Otros créditos"},{key:"importe_total_de_la_deuda",label:"Importe de la deuda",conditional:!0},{key:"proposito_del_prestamo",label:"Propósito del préstamo"},{key:"creditos_cantidad_a_solicitar",label:"Importe a solicitar"},{key:"en_cuantos_meses_deseas_devolverlo",label:"Plazo de devolución"}],Se=K.map(e=>({key:e.key,label:e.label,conditional:!!e.condition})),G=new Intl.DateTimeFormat("es-ES",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"});function r(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML.replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function C(e){return e==="all"?null:e}async function ae(e,t,a){const{data:s,error:n}=await _.rpc("admin_get_stats",{p_password:e,p_since:t.since,p_until:t.until,p_source:C(a)}).single();if(n||!s)throw n??new Error("No data");return s}async function Ee(e,t,a){const{data:s,error:n}=await _.rpc("admin_get_funnel_overview",{p_password:e,p_since:t.since,p_until:t.until,p_source:C(a)}).single();if(n||!s)throw n??new Error("No data");return s}async function ke(e,t,a){const{data:s,error:n}=await _.rpc("admin_get_funnel_steps",{p_password:e,p_since:t.since,p_until:t.until,p_source:C(a)});if(n)throw n;return s??[]}async function Le(e,t,a,s){const{data:n,error:o}=await _.rpc("admin_list_leads",{p_password:e,p_limit:F,p_offset:s*F,p_since:t.since,p_until:t.until,p_source:C(a)});if(o)throw o;return n??[]}async function Ie(e,t){const{data:a,error:s}=await _.rpc("admin_get_witme_applications",{p_password:e,p_limit:R,p_offset:t*R});if(s)throw s;return a??[]}async function we(e,t,a){const{data:s,error:n}=await _.rpc("admin_get_offer_clicks",{p_password:e,p_since:t.since,p_until:t.until,p_source:C(a)});if(n)throw n;return s??[]}let ne=[];async function Te(e){const{data:t,error:a}=await _.rpc("admin_get_scoring_rules",{p_password:e});if(a)throw a;return t??[]}async function Ce(e,t,a,s,n){const{error:o}=await _.rpc("admin_update_scoring_rule",{p_password:e,p_key:t,p_config:a,p_weight:s,p_active:n});if(o)throw o}async function Ae(e,t){const{error:a}=await _.rpc("admin_reset_scoring_rule",{p_password:e,p_key:t});if(a)throw a}async function Oe(e){const{error:t}=await _.rpc("admin_reset_all_scoring_rules",{p_password:e});if(t)throw t}async function qe(e,t,a){const{data:s,error:n}=await _.rpc("admin_get_field_stats",{p_password:e,p_since:t.since,p_until:t.until,p_source:a});if(n)throw n;return s}function T(e){v.innerHTML=`
  ...
  `,document.getElementById("login-form").addEventListener("submit",async t=>{t.preventDefault();const a=document.getElementById("pw-input").value;try{await ae(a,N("all"),"all"),sessionStorage.setItem(w,a),k(a)}catch{T("Contraseña incorrecta.")}})}function m(e,t){return`<div class="admin-stat"><span class="admin-stat-value">${t}</span><span class="admin-stat-label">${e}</span></div>`}function O(e,t,a,s){const n=a>0?Math.round(t/a*100):0;return`
  ...
  `}function Z(e){return e>=60?"band-excelente":e>=35?"band-bueno":e>=15?"band-regular":"band-bajo"}function De(e,t,a){var p;const s=new Map(t.map(l=>[l.question_key,Number(l.reached)])),n=e.engaged_visits;let o="",d=(p=a[0])==null?void 0:p.key;return a.forEach((l,$)=>{const u=s.get(l.key)??0,b=n>0?Math.round(u/n*100):0;let c="";if($>0&&!l.conditional){const y=s.get(d)??0;if(y>0){const S=Math.round((1-u/y)*100),A=S>=25?"high":S>=10?"mid":"low";c=S>0?`<span class="funnel-drop funnel-drop-${A}">-${S}% respecto al paso anterior</span>`:'<span class="funnel-drop funnel-drop-low">sin caída</span>'}}o+=`
    ...
    `,l.conditional||(d=l.key)}),o}function k(e){H==="scoring"?P(e):H==="fieldstats"?oe(e):I(e)}function x(e){return`
  ...
  `}function Q(e){document.querySelectorAll(".admin-tab-btn").forEach(t=>{t.addEventListener("click",()=>{H=t.dataset.tab,k(e)})}),document.getElementById("refresh-btn").addEventListener("click",()=>k(e)),document.getElementById("logout-btn").addEventListener("click",()=>{sessionStorage.removeItem(w),T()})}function se(e){return`
    ...
    <section class="admin-card admin-period-bar">
      ...
      <p class="admin-period-label">${r($e(h,e))}</p>
    </section>
  `}function ie(e){var t;document.querySelectorAll(".admin-period-btn[data-source]").forEach(a=>{a.addEventListener("click",()=>{g=a.dataset.source,f=0,k(e)})}),document.querySelectorAll(".admin-period-btn[data-preset]").forEach(a=>{a.addEventListener("click",()=>{h=a.dataset.preset,f=0,k(e)})}),(t=document.getElementById("period-apply-btn"))==null||t.addEventListener("click",()=>{j=document.getElementById("period-from").value||j,z=document.getElementById("period-to").value||z,h="custom",f=0,k(e)})}async function I(e){var a,s;v.innerHTML='<div class="admin-shell"><p class="admin-loading">Cargando…</p></div>';const t=N(h);try{const[n,o,d,p,l,$]=await Promise.all([ae(e,t,g),Le(e,t,g,f),Ee(e,t,g),ke(e,t,g),Ie(e,E),we(e,t,g)]),u=n.band_excelente+n.band_bueno+n.band_regular+n.band_bajo,b=g==="solicitud"?Se:ye,c=((a=o[0])==null?void 0:a.total_count)??0,y=Math.max(1,Math.ceil(c/F)),S=((s=l[0])==null?void 0:s.total_count)??0,A=Math.max(1,Math.ceil(S/R));v.innerHTML=`
              ...
              <tbody>
                ${o.map(i=>`
                  <tr>
                    ...
                    <td>${i.offer_clicks&&i.offer_clicks.length>0?i.offer_clicks.map(L=>r(B[L]??L)).join(", "):"—"}</td>
                  </tr>
                `).join("")}
        ...
        <section class="admin-card">
          ...
          <div class="admin-table-scroll">
            <table class="admin-table">
              ...
              <tbody>
                ${l.map(i=>{const L=[];i.witme_message!=null&&L.push(`Mensaje: ${JSON.stringify(i.witme_message)}`),i.witme_redirect_url&&L.push(`Redirect URL: ${i.witme_redirect_url}`);const W=L.join(`
`);return`
                  <tr>
                    <td>${G.format(new Date(i.created_at))}</td>
                    ...
                    <td>${i.score??"—"}</td>
                    <td>${i.approval_probability!=null?`<span class="admin-badge ${Z(i.approval_probability)}">${i.approval_probability}%</span>`:"—"}</td>
                    <td>${i.offer_clicks&&i.offer_clicks.length>0?`✅ ${i.offer_clicks.map(U=>r(B[U]??U)).join(", ")}`:"—"}</td>
                  </tr>
                `}).join("")}
          ...
          <div class="admin-table-scroll">
            <table class="admin-table">
              ...
              <tbody>
                ${$.map(i=>`
                  <tr>
                    <td>${r(B[i.offer_id]??i.offer_id)}</td>
                    <td>${i.clicks}</td>
                  </tr>
                `).join("")}
    ...
    `,Q(e),ie(e),document.getElementById("leads-prev-btn").addEventListener("click",()=>{f>0&&(f--,I(e))}),document.getElementById("leads-next-btn").addEventListener("click",()=>{f++,I(e)}),document.getElementById("witme-prev-btn").addEventListener("click",()=>{E>0&&(E--,I(e))}),document.getElementById("witme-next-btn").addEventListener("click",()=>{E++,I(e)})}catch(n){const o=n instanceof Error?n.message:String(n);o.toLowerCase().includes("unauthorized")?(sessionStorage.removeItem(w),T("Tu sesión ha caducado o la contraseña ya no es válida.")):(v.innerHTML=`
        <div class="admin-shell">
      ...
      `,document.getElementById("retry-btn").addEventListener("click",()=>I(e)))}}function je(e,t,a){return a?`${e} +`:`${e} – ${t}`}function D(e,t,a,s,n,o,d){return`
  ...
  `}function ze(e){const t=e.config;if(typeof t.value=="number"&&Object.keys(t).length===1)return D(e.key,"value","Puntos base",t.value,300,850,5);if(Array.isArray(t.buckets)){const a=t.buckets;return a.map((s,n)=>D(e.key,`bucket:${n}`,je(s[0],s[1],n===a.length-1),s[2],-200,200,5)).join("")}return Object.entries(t).map(([a,s])=>D(e.key,`opt:${a}`,a,Number(s),-200,200,5)).join("")}function Pe(e){return`
    <div class="scoring-rule-card" data-rule-card="${e.key}">
      <div class="scoring-rule-header">
        <div>
          <p class="scoring-rule-label">${r(e.label)}</p>
          <p class="scoring-rule-used-by">Usado en: ${r(he[e.key]??"—")} · clave: <code>${r(e.key)}</code></p>
        </div>
  ...
  `}function Be(e){var t;document.querySelectorAll(".scoring-slider").forEach(a=>{a.addEventListener("input",()=>{var n;const s=(n=a.closest(".scoring-slider-row"))==null?void 0:n.querySelector(".scoring-slider-value");s&&(s.textContent=a.value)})}),document.querySelectorAll("[data-save-rule]").forEach(a=>{a.addEventListener("click",async()=>{const s=a.dataset.saveRule,n=ne.find(c=>c.key===s),o=document.querySelector(`[data-rule-card="${s}"]`);if(!n||!o)return;const d=o.querySelector(".scoring-rule-status"),p=new Map;o.querySelectorAll("input[data-field]").forEach(c=>{p.set(c.dataset.field,c.type==="checkbox"?String(c.checked):c.value)});const l=Number(p.get("weight")),$=p.get("active")==="true",u=n.config;let b;typeof u.value=="number"&&Object.keys(u).length===1?b={value:Number(p.get("value"))}:Array.isArray(u.buckets)?b={buckets:u.buckets.map((c,y)=>[c[0],c[1],Number(p.get(`bucket:${y}`))])}:(b={},Object.keys(u).forEach(c=>{b[c]=Number(p.get(`opt:${c}`))})),a.disabled=!0,d.textContent="Guardando…",d.className="scoring-rule-status";try{await Ce(e,s,b,l,$),n.config=b,n.weight=l,n.active=$,d.textContent="✓ Guardado",d.className="scoring-rule-status ok",setTimeout(()=>{d.textContent=""},2500)}catch{d.textContent="Error al guardar",d.className="scoring-rule-status error"}finally{a.disabled=!1}})}),document.querySelectorAll("[data-reset-rule]").forEach(a=>{a.addEventListener("click",async()=>{const s=a.dataset.resetRule,n=document.querySelector(`[data-rule-card="${s}"]`);if(!n||!confirm("¿Restaurar esta regla a sus valores por defecto? Se aplicará de inmediato."))return;const o=n.querySelector(".scoring-rule-status");a.disabled=!0,o.textContent="Restaurando…",o.className="scoring-rule-status";try{await Ae(e,s),await P(e)}catch{o.textContent="Error al restaurar",o.className="scoring-rule-status error",a.disabled=!1}})}),(t=document.getElementById("reset-all-rules-btn"))==null||t.addEventListener("click",async()=>{if(confirm("¿Restaurar TODAS las reglas de scoring a sus valores por defecto? Esto sobrescribe cualquier ajuste manual y se aplica de inmediato a las puntuaciones reales."))try{await Oe(e),await P(e)}catch{alert("No se ha podido restaurar. Inténtalo de nuevo.")}})}async function P(e){v.innerHTML='<div class="admin-shell"><p class="admin-loading">Cargando…</p></div>';try{const t=await Te(e);ne=t,v.innerHTML=`
    ...
    `,Q(e),Be(e)}catch(t){const a=t instanceof Error?t.message:String(t);a.toLowerCase().includes("unauthorized")?(sessionStorage.removeItem(w),T("Tu sesión ha caducado o la contraseña ya no es válida.")):(v.innerHTML=`
        <div class="admin-shell">
          <p class="admin-error">Ha ocurrido un error inesperado cargando el algoritmo: ${r(a)}</p>
          <button class="admin-btn-ghost" id="retry-btn">Reintentar</button>
        </div>
      `,document.getElementById("retry-btn").addEventListener("click",()=>P(e)))}}const Y=new Intl.NumberFormat("es-ES",{maximumFractionDigits:1});function q(e,t){return e==null?"—":t?`${Y.format(e)} €`:Y.format(e)}function Fe(e,t,a){const s=_e.has(t);return`
  ...
  `}function Re(e,t,a,s){const n=t.reduce((o,d)=>o+d.count,0);return`
    <div class="fieldstat-card">
      <p class="fieldstat-label">${r(e)}</p>
      ${t.map(o=>{const d=n>0?Math.round(o.count/n*100):0;return`
            <div class="admin-band-row">
              <span class="admin-band-label">${r(pe(a,s,o.value))}</span>
  ...
  `}function He(e,t){const a=e==="solicitud"?ge:be;return`
    <section class="admin-card">
      ...
      <div class="fieldstats-grid">
        ${Object.entries(t.numeric).map(([n,o])=>Fe(a[n]??n,n,o)).join("")}
        ${Object.entries(t.categorical).map(([n,o])=>Re(a[n]??n,o,e,n)).join("")}
      </div>
    </section>
  `}async function oe(e){v.innerHTML='<div class="admin-shell"><p class="admin-loading">Cargando…</p></div>';const t=N(h),a=g==="solicitud"?["solicitud"]:g==="quiz"?["quiz"]:["quiz","solicitud"];try{const s=await Promise.all(a.map(n=>qe(e,t,n)));v.innerHTML=`
      <div class="admin-shell">
        ${x("fieldstats")}
        ...
        ${a.map((n,o)=>He(n,s[o])).join("")}
      </div>
    `,Q(e),ie(e)}catch(s){const n=s instanceof Error?s.message:String(s);n.toLowerCase().includes("unauthorized")?(sessionStorage.removeItem(w),T("Tu sesión ha caducado o la contraseña ya no es válida.")):(v.innerHTML=`
        <div class="admin-shell">
          <p class="admin-error">Ha ocurrido un error inesperado cargando las estadísticas: ${r(n)}</p>
          <button class="admin-btn-ghost" id="retry-btn">Reintentar</button>
        </div>
      `,document.getElementById("retry-btn").addEventListener("click",()=>oe(e)))}}const J=sessionStorage.getItem(w);J?k(J):T();
