import{C as re,c as de}from"./validation-DnU8phWm.js";import{q as ce}from"./questions-D4Dn-9Ya.js";import{W as K}from"./witmeQuestions-D841kSbQ.js";const Q={witme_featured:"Witme (oferta destacada)",...Object.fromEntries(re.map(e=>[e.id,e.name]))};function M(e){if(e in Q)return Q[e];const t=e.match(/^witme_featured_(\d+)$/);return t?`Witme (oferta destacada ${t[1]})`:e}function X(e){const t={};for(const a of e)a.options&&(t[a.key]=Object.fromEntries(a.options.map(n=>[n.value,n.label])));return t}const le=X(ce),ue=X(K),me={si:"Sí",no:"No"};function pe(e,t,a){const n=e==="solicitud"?ue[t]:le[t];return(n==null?void 0:n[a])??me[a]??a}const be={ingreso_mensual:"Ingreso mensual",importe_total_de_la_deuda:"Deuda total (entre quienes tienen)",creditos_cantidad_a_solicitar:"Importe solicitado",age:"Edad",esta_en_asnef:"En ASNEF",antiguedad_laboral:"Antigüedad laboral",tienes_otros_creditos:"Tiene otras deudas",proposito_del_prestamo:"Propósito del préstamo",fuente_principal_de_ingreso:"Fuente de ingresos",tienes_vivienda_en_propiedad:"Vivienda en propiedad",en_cuantos_meses_deseas_devolverlo:"Plazo de devolución"},ge={monthlyIncome:"Ingreso mensual",totalDebtAmount:"Deuda total (entre quienes tienen)",requestedAmount:"Importe solicitado",numberOfdependents:"Personas a cargo",age:"Edad",incomeSource:"Fuente de ingresos",hasOwnedHouse:"Situación de vivienda",badCreditHistory:"En ASNEF",hasOtherLoans:"Tiene otras deudas",loanPurpose:"Propósito del préstamo",hasOwnVehicle:"Tiene vehículo propio",hasBankAccount:"Tiene cuenta bancaria",maritalStatus:"Estado civil",educationLevel:"Nivel de estudios",gender:"Género",countryOfBirth:"País de nacimiento",state:"Comunidad autónoma"},_e=new Set(["ingreso_mensual","importe_total_de_la_deuda","creditos_cantidad_a_solicitar","monthlyIncome","totalDebtAmount","requestedAmount"]),fe="https://pgyaigdsedkdqvhtexrz.supabase.co",ve="sb_publishable_yL99vHU_H5kGZ3SMuPS0hA_GJ_TWTMr",g=de(fe,ve),E="cs_admin_pw",p=document.getElementById("admin-root");function ee(e){return e.toISOString().slice(0,10)}const te=new Date;let v="all",z=ee(te),D=ee(te),_="all";const R=25;let f=0;const H=25;let y=0,O="dashboard";const he={base:"Quiz corto + Solicitud",ingreso_mensual:"Quiz corto + Solicitud",otros_creditos:"Quiz corto + Solicitud",asnef:"Quiz corto + Solicitud",ratio_deuda_ingreso:"Quiz corto + Solicitud",edad:"Quiz corto + Solicitud",fuente_ingreso:"Quiz corto",antiguedad_laboral:"Quiz corto",vivienda_propiedad:"Quiz corto",solicitud_fuente_ingreso:"Solicitud",solicitud_antiguedad:"Solicitud",solicitud_vivienda:"Solicitud",solicitud_dependientes:"Solicitud",aprobacion_base:"Probabilidad de aprobación (quiz + solicitud)",aprobacion_ratio_importe:"Probabilidad de aprobación (quiz + solicitud)"},x={all:"Todos",quiz:"Quiz corto",solicitud:"Solicitud completa"};function j(e){const t=new Date;if(e==="today")return{since:new Date(t.getFullYear(),t.getMonth(),t.getDate(),0,0,0,0).toISOString(),until:t.toISOString()};if(e==="7d")return{since:new Date(t.getTime()-6048e5).toISOString(),until:t.toISOString()};if(e==="custom"){const a=new Date(`${z}T00:00:00`),n=new Date(`${D}T23:59:59.999`);return a.getTime()>n.getTime()?{since:n.toISOString(),until:a.toISOString()}:{since:a.toISOString(),until:n.toISOString()}}return{since:"2000-01-01T00:00:00.000Z",until:t.toISOString()}}const U=new Intl.DateTimeFormat("es-ES",{day:"2-digit",month:"short",year:"numeric"});function $e(e,t){return e==="all"?"Todo el histórico":`${U.format(new Date(t.since))} – ${U.format(new Date(t.until))}`}const ye=[{key:"fecha_de_nacimiento",label:"Fecha de nacimiento"},{key:"codigo_postal",label:"Código postal"},{key:"fuente_principal_de_ingreso",label:"Fuente de ingresos"},{key:"antiguedad_laboral",label:"Antigüedad laboral",conditional:!0},{key:"tienes_vivienda_en_propiedad",label:"Vivienda en propiedad"},{key:"ingreso_mensual",label:"Ingreso mensual"},{key:"esta_en_asnef",label:"Asnef"},{key:"tienes_otros_creditos",label:"Otros créditos"},{key:"importe_total_de_la_deuda",label:"Importe de la deuda",conditional:!0},{key:"proposito_del_prestamo",label:"Propósito del préstamo"},{key:"creditos_cantidad_a_solicitar",label:"Importe a solicitar"},{key:"en_cuantos_meses_deseas_devolverlo",label:"Plazo de devolución"}],Se=K.map(e=>({key:e.key,label:e.label,conditional:!!e.condition})),V=new Intl.DateTimeFormat("es-ES",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"});function d(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML.replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function T(e){return e==="all"?null:e}async function ae(e,t,a){const{data:n,error:s}=await g.rpc("admin_get_stats",{p_password:e,p_since:t.since,p_until:t.until,p_source:T(a)}).single();if(s||!n)throw s??new Error("No data");return n}async function Ee(e,t,a){const{data:n,error:s}=await g.rpc("admin_get_funnel_overview",{p_password:e,p_since:t.since,p_until:t.until,p_source:T(a)}).single();if(s||!n)throw s??new Error("No data");return n}async function ke(e,t,a){const{data:n,error:s}=await g.rpc("admin_get_funnel_steps",{p_password:e,p_since:t.since,p_until:t.until,p_source:T(a)});if(s)throw s;return n??[]}async function Le(e,t,a,n){const{data:s,error:o}=await g.rpc("admin_list_leads",{p_password:e,p_limit:R,p_offset:n*R,p_since:t.since,p_until:t.until,p_source:T(a)});if(o)throw o;return s??[]}async function we(e,t){const{data:a,error:n}=await g.rpc("admin_get_witme_applications",{p_password:e,p_limit:H,p_offset:t*H});if(n)throw n;return a??[]}async function Ie(e){const{data:t,error:a}=await g.rpc("admin_get_witme_response_stats",{p_password:e}).single();if(a||!t)throw a??new Error("No data");return t}function I(e){return e==null?"—":`${(e/1e3).toFixed(1)} s`}function G(e){return e==null?"—":`${e}%`}async function Te(e,t,a){const{data:n,error:s}=await g.rpc("admin_get_offer_clicks",{p_password:e,p_since:t.since,p_until:t.until,p_source:T(a)});if(s)throw s;return n??[]}let ne=[];async function Ce(e){const{data:t,error:a}=await g.rpc("admin_get_scoring_rules",{p_password:e});if(a)throw a;return t??[]}async function Ae(e,t,a,n,s){const{error:o}=await g.rpc("admin_update_scoring_rule",{p_password:e,p_key:t,p_config:a,p_weight:n,p_active:s});if(o)throw o}async function Oe(e,t){const{error:a}=await g.rpc("admin_reset_scoring_rule",{p_password:e,p_key:t});if(a)throw a}async function qe(e){const{error:t}=await g.rpc("admin_reset_all_scoring_rules",{p_password:e});if(t)throw t}async function ze(e,t,a){const{data:n,error:s}=await g.rpc("admin_get_field_stats",{p_password:e,p_since:t.since,p_until:t.until,p_source:a});if(s)throw s;return n}function k(e){p.innerHTML=`
  ...
  `,document.getElementById("login-form").addEventListener("submit",async t=>{t.preventDefault();const a=document.getElementById("pw-input").value;try{await ae(a,j("all"),"all"),sessionStorage.setItem(E,a),S(a)}catch{k("Contraseña incorrecta.")}})}function l(e,t){return`<div class="admin-stat"><span class="admin-stat-value">${t}</span><span class="admin-stat-label">${e}</span></div>`}function C(e,t,a,n){const s=a>0?Math.round(t/a*100):0;return`
  ...
  `}function Z(e){return e>=60?"band-excelente":e>=35?"band-bueno":e>=15?"band-regular":"band-bajo"}function De(e,t,a){var m;const n=new Map(t.map(u=>[u.question_key,Number(u.reached)])),s=e.engaged_visits;let o="",r=(m=a[0])==null?void 0:m.key;return a.forEach((u,h)=>{const b=n.get(u.key)??0,i=s>0?Math.round(b/s*100):0;let c="";if(h>0&&!u.conditional){const $=n.get(r)??0;if($>0){const L=Math.round((1-b/$)*100),oe=L>=25?"high":L>=10?"mid":"low";c=L>0?`<span class="funnel-drop funnel-drop-${oe}">-${L}% respecto al paso anterior</span>`:'<span class="funnel-drop funnel-drop-low">sin caída</span>'}}o+=`
    ...
    `,u.conditional||(r=u.key)}),o}function S(e){O==="scoring"?P(e):O==="fieldstats"?ie(e):O==="leads"?w(e):se(e)}function B(e){return`
  ...
  `}function F(e){document.querySelectorAll(".admin-tab-btn").forEach(t=>{t.addEventListener("click",()=>{O=t.dataset.tab,S(e)})}),document.getElementById("refresh-btn").addEventListener("click",()=>S(e)),document.getElementById("logout-btn").addEventListener("click",()=>{sessionStorage.removeItem(E),k()})}function N(e){return`
    ...
    <section class="admin-card admin-period-bar">
      ...
      <div class="admin-period-custom ${v==="custom"?"active":""}">
        ...
        <span>–</span>
        <input type="date" id="period-to" value="${D}" />
        <button class="admin-btn-ghost" id="period-apply-btn">Aplicar</button>
      </div>
      <p class="admin-period-label">${d($e(v,e))}</p>
    </section>
  `}function W(e){var t;document.querySelectorAll(".admin-period-btn[data-source]").forEach(a=>{a.addEventListener("click",()=>{_=a.dataset.source,f=0,S(e)})}),document.querySelectorAll(".admin-period-btn[data-preset]").forEach(a=>{a.addEventListener("click",()=>{v=a.dataset.preset,f=0,S(e)})}),(t=document.getElementById("period-apply-btn"))==null||t.addEventListener("click",()=>{z=document.getElementById("period-from").value||z,D=document.getElementById("period-to").value||D,v="custom",f=0,S(e)})}async function se(e){p.innerHTML='<div class="admin-shell"><p class="admin-loading">Cargando…</p></div>';const t=j(v);try{const[a,n,s,o]=await Promise.all([ae(e,t,_),Ee(e,t,_),ke(e,t,_),Te(e,t,_)]),r=a.band_excelente+a.band_bueno+a.band_regular+a.band_bajo,m=_==="solicitud"?Se:ye;p.innerHTML=`
    ...
    `,F(e),W(e)}catch(a){const n=a instanceof Error?a.message:String(a);n.toLowerCase().includes("unauthorized")?(sessionStorage.removeItem(E),k("Tu sesión ha caducado o la contraseña ya no es válida.")):(p.innerHTML=`
        <div class="admin-shell">
      ...
      `,document.getElementById("retry-btn").addEventListener("click",()=>se(e)))}}async function w(e){var a,n;p.innerHTML='<div class="admin-shell"><p class="admin-loading">Cargando…</p></div>';const t=j(v);try{const[s,o,r]=await Promise.all([Le(e,t,_,f),we(e,y),Ie(e)]),m=((a=s[0])==null?void 0:a.total_count)??0,u=Math.max(1,Math.ceil(m/R)),h=((n=o[0])==null?void 0:n.total_count)??0,b=Math.max(1,Math.ceil(h/H));p.innerHTML=`
      <div class="admin-shell">
        ${B("leads")}
              ...
              <tbody>
                ${s.map(i=>`
                  <tr>
                    ...
                    <td>${i.offer_clicks&&i.offer_clicks.length>0?i.offer_clicks.map(c=>d(M(c))).join(", "):"—"}</td>
                  </tr>
                `).join("")}
        ...
        <section class="admin-card">
          ...
          <section class="admin-stats-grid admin-stats-grid-compact">
            ${l("% Aceptados (con oferta)",G(r.pct_accepted))}
            ${l("% Rechazados por Witme",G(r.pct_failed))}
...
`);return`
                  <tr>
                    ...
                    <td>${i.score??"—"}</td>
                    <td>${i.approval_probability!=null?`<span class="admin-badge ${Z(i.approval_probability)}">${i.approval_probability}%</span>`:"—"}</td>
                    <td>${i.offer_clicks&&i.offer_clicks.length>0?`✅ ${i.offer_clicks.map(L=>d(M(L))).join(", ")}`:"—"}</td>
                  </tr>
                `}).join("")}
    ...
    `,F(e),W(e),document.getElementById("leads-prev-btn").addEventListener("click",()=>{f>0&&(f--,w(e))}),document.getElementById("leads-next-btn").addEventListener("click",()=>{f++,w(e)}),document.getElementById("witme-prev-btn").addEventListener("click",()=>{y>0&&(y--,w(e))}),document.getElementById("witme-next-btn").addEventListener("click",()=>{y++,w(e)})}catch(s){const o=s instanceof Error?s.message:String(s);o.toLowerCase().includes("unauthorized")?(sessionStorage.removeItem(E),k("Tu sesión ha caducado o la contraseña ya no es válida.")):(p.innerHTML=`
        <div class="admin-shell">
          <p class="admin-error">Ha ocurrido un error inesperado cargando el panel: ${d(o)}</p>
          <button class="admin-btn-ghost" id="retry-btn">Reintentar</button>
        </div>
      `,document.getElementById("retry-btn").addEventListener("click",()=>w(e)))}}function Pe(e,t,a){return a?`${e} +`:`${e} – ${t}`}function q(e,t,a,n,s,o,r){return`
  ...
  `}function je(e){const t=e.config;if(typeof t.value=="number"&&Object.keys(t).length===1)return q(e.key,"value","Puntos base",t.value,300,850,5);if(Array.isArray(t.buckets)){const a=t.buckets;return a.map((n,s)=>q(e.key,`bucket:${s}`,Pe(n[0],n[1],s===a.length-1),n[2],-200,200,5)).join("")}return Object.entries(t).map(([a,n])=>q(e.key,`opt:${a}`,a,Number(n),-200,200,5)).join("")}function Be(e){return`
    <div class="scoring-rule-card" data-rule-card="${e.key}">
      <div class="scoring-rule-header">
        <div>
          <p class="scoring-rule-label">${d(e.label)}</p>
          <p class="scoring-rule-used-by">Usado en: ${d(he[e.key]??"—")} · clave: <code>${d(e.key)}</code></p>
        </div>
  ...
  `}function Fe(e){var t;document.querySelectorAll(".scoring-slider").forEach(a=>{a.addEventListener("input",()=>{var s;const n=(s=a.closest(".scoring-slider-row"))==null?void 0:s.querySelector(".scoring-slider-value");n&&(n.textContent=a.value)})}),document.querySelectorAll("[data-save-rule]").forEach(a=>{a.addEventListener("click",async()=>{const n=a.dataset.saveRule,s=ne.find(c=>c.key===n),o=document.querySelector(`[data-rule-card="${n}"]`);if(!s||!o)return;const r=o.querySelector(".scoring-rule-status"),m=new Map;o.querySelectorAll("input[data-field]").forEach(c=>{m.set(c.dataset.field,c.type==="checkbox"?String(c.checked):c.value)});const u=Number(m.get("weight")),h=m.get("active")==="true",b=s.config;let i;typeof b.value=="number"&&Object.keys(b).length===1?i={value:Number(m.get("value"))}:Array.isArray(b.buckets)?i={buckets:b.buckets.map((c,$)=>[c[0],c[1],Number(m.get(`bucket:${$}`))])}:(i={},Object.keys(b).forEach(c=>{i[c]=Number(m.get(`opt:${c}`))})),a.disabled=!0,r.textContent="Guardando…",r.className="scoring-rule-status";try{await Ae(e,n,i,u,h),s.config=i,s.weight=u,s.active=h,r.textContent="✓ Guardado",r.className="scoring-rule-status ok",setTimeout(()=>{r.textContent=""},2500)}catch{r.textContent="Error al guardar",r.className="scoring-rule-status error"}finally{a.disabled=!1}})}),document.querySelectorAll("[data-reset-rule]").forEach(a=>{a.addEventListener("click",async()=>{const n=a.dataset.resetRule,s=document.querySelector(`[data-rule-card="${n}"]`);if(!s||!confirm("¿Restaurar esta regla a sus valores por defecto? Se aplicará de inmediato."))return;const o=s.querySelector(".scoring-rule-status");a.disabled=!0,o.textContent="Restaurando…",o.className="scoring-rule-status";try{await Oe(e,n),await P(e)}catch{o.textContent="Error al restaurar",o.className="scoring-rule-status error",a.disabled=!1}})}),(t=document.getElementById("reset-all-rules-btn"))==null||t.addEventListener("click",async()=>{if(confirm("¿Restaurar TODAS las reglas de scoring a sus valores por defecto? Esto sobrescribe cualquier ajuste manual y se aplica de inmediato a las puntuaciones reales."))try{await qe(e),await P(e)}catch{alert("No se ha podido restaurar. Inténtalo de nuevo.")}})}async function P(e){p.innerHTML='<div class="admin-shell"><p class="admin-loading">Cargando…</p></div>';try{const t=await Ce(e);ne=t,p.innerHTML=`
    ...
    `,F(e),Fe(e)}catch(t){const a=t instanceof Error?t.message:String(t);a.toLowerCase().includes("unauthorized")?(sessionStorage.removeItem(E),k("Tu sesión ha caducado o la contraseña ya no es válida.")):(p.innerHTML=`
        <div class="admin-shell">
          <p class="admin-error">Ha ocurrido un error inesperado cargando el algoritmo: ${d(a)}</p>
          <button class="admin-btn-ghost" id="retry-btn">Reintentar</button>
        </div>
      `,document.getElementById("retry-btn").addEventListener("click",()=>P(e)))}}const Y=new Intl.NumberFormat("es-ES",{maximumFractionDigits:1});function A(e,t){return e==null?"—":t?`${Y.format(e)} €`:Y.format(e)}function Me(e,t,a){const n=_e.has(t);return`
  ...
  `}function Re(e,t,a,n){const s=t.reduce((o,r)=>o+r.count,0);return`
    <div class="fieldstat-card">
      <p class="fieldstat-label">${d(e)}</p>
      ${t.map(o=>{const r=s>0?Math.round(o.count/s*100):0;return`
            <div class="admin-band-row">
              <span class="admin-band-label">${d(pe(a,n,o.value))}</span>
  ...
  `}function He(e,t){const a=e==="solicitud"?ge:be;return`
    <section class="admin-card">
      <p class="admin-card-title">${e==="solicitud"?"Solicitud completa":"Quiz corto"} (${t.count} sesiones)</p>
      <div class="fieldstats-grid">
        ${Object.entries(t.numeric).map(([s,o])=>Me(a[s]??s,s,o)).join("")}
        ${Object.entries(t.categorical).map(([s,o])=>Re(a[s]??s,o,e,s)).join("")}
      </div>
    </section>
  `}async function ie(e){p.innerHTML='<div class="admin-shell"><p class="admin-loading">Cargando…</p></div>';const t=j(v),a=_==="solicitud"?["solicitud"]:_==="quiz"?["quiz"]:["quiz","solicitud"];try{const n=await Promise.all(a.map(s=>ze(e,t,s)));p.innerHTML=`
      <div class="admin-shell">
        ${B("fieldstats")}
        ...
        ${a.map((s,o)=>He(s,n[o])).join("")}
      </div>
    `,F(e),W(e)}catch(n){const s=n instanceof Error?n.message:String(n);s.toLowerCase().includes("unauthorized")?(sessionStorage.removeItem(E),k("Tu sesión ha caducado o la contraseña ya no es válida.")):(p.innerHTML=`
        <div class="admin-shell">
          <p class="admin-error">Ha ocurrido un error inesperado cargando las estadísticas: ${d(s)}</p>
          <button class="admin-btn-ghost" id="retry-btn">Reintentar</button>
        </div>
      `,document.getElementById("retry-btn").addEventListener("click",()=>ie(e)))}}const J=sessionStorage.getItem(E);J?S(J):k();
